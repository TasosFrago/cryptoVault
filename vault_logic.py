...
VAULT_KEY_PASSWORD = "lifting-parking-skyline" # for vault's private key

class Vault:
    def __init__(self):
        os.makedirs(BASE_STORAGE_PATH, exists_ok = True)
        try:
            self.private_key = crypto_utils.load_private_key(VAULT_PRIVATE_KEY_PATH, VAULT_KEY_PASSWORD)
            self.public_key = crypto_utils.load_public_key_from_certificate_file(VAULT_CERT_PATH)
        except Exception as e:
            raise Exception(f"Failed to initialize Vault keys: {e}")
        print("Vault initialized with its cryptographic keys.")
        
    def register_user(self, username, cert_file_path):
        user_id = crypto_utils.load_public_key_and_id_from_certificate_file(cert_file_path)[1]
        folder_name = f"{user_id}-{username}"
        user_folder = os.path.join(BASE_STORAGE_PATH, folder_name)
        os.makedirs(user_folder, exists_ok = True)
        print(f"User '{username}' registered. Storage folder created at {user_folder}")
        return True
    
    def store_file(self, folder_name, user_uploaded_encrypted_file_path, user_uploaded_encrypted_sym_key_path, original_filename_tag):
        ...
        try:
            shutil.copy(user_uploaded_encrypted_file_path, stored_encrypted_data_path)
            shutil.copy(user_uploaded_encrypted_sym_key_path, stored_encrypted_sym_key_path)
    ...
    def retrieve_file(self, folder_name, original_filename_tag):
        username = folder_name.split("-", 1)[1]
        user_folder = os.path.join(BASE_STORAGE_PATH, folder_name)
        if not os.path.exists(user_folder):
            print(f"Error: User '{username}' not registered.")
